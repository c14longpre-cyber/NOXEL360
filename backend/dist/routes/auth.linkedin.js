...
function startLinkedInAuth(res) {
    const state = crypto_1.default.randomBytes(24).toString("hex");
    const { url } = (0, linkedinAuth_1.buildLinkedInAuthorizeUrl)(state);
    ...
    try {
        ...
        if (errorParam || errorDescription) {
            const redirectParams = new URLSearchParams({
                ...
                description: errorDescription || "User cancelled or provider returned an error.",
            });
        ...
        const redirectParams = new URLSearchParams({
            ...
            avatarUrl: "picture" in userInfoJson ? String(userInfoJson.picture || "") : "",
        });
        return res.redirect(`${frontendUrl}/auth/callback?${redirectParams.toString()}`);
    }
    catch (error) {
