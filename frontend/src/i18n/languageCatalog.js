 ...
 */
export const FULLY_TRANSLATED_CODES = new Set([
...
function getTranslationStatus(code) {
    if (FULLY_TRANSLATED_CODES.has(code))
        return "complete";
    return "untranslated";
}
function makeLanguageOption(code, entry) {
...
export const LANGUAGE_CATALOG = buildLanguageCatalog();
/** Map O(1) pour la recherche par code */
export const LANGUAGE_BY_CODE = new Map(LANGUAGE_CATALOG.map((lang) => [lang.code, lang]));
