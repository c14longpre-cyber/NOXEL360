...
export function getCountryByISO2(iso2) {
    ...
    return (Object.values(COUNTRY_DATA).find((country) => country.iso2 === key) || null);
}
export function getCountryByISO3(iso3) {
    if (!iso3)
        return null;
    const key = iso3.toUpperCase();
    return (Object.values(COUNTRY_DATA).find((country) => country.iso3 === key) || null);
}
...
export function getCountryPrimaryLanguageCode(country) {
    if (!country)
        return null;
    return (country.languages?.find((lang) => lang.official)?.code ||
