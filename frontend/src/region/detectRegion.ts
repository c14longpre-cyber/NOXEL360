 ...
 */
const LANGUAGE_TO_DEFAULT_REGION: Record<string, string> = {
  en: "US",
  fr: "FR",
  es: "ES",
  de: "DE",
...
function getStoredRegion(): string | null {
  ...
  try {
    ...
    if (raw && isSupportedRegion(raw)) return raw.toUpperCase();
  } catch {
...
function extractRegionFromNavigatorLanguage(lang: string): string | null {
  if (!lang) return null;
  // "fr-CA" → "CA"
  ...
  if (mapped && isSupportedRegion(mapped)) return mapped;
  return null;
}
...
export function persistRegion(code: string): void {
