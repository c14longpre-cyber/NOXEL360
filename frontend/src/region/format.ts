...
export type FormatContext = {
  /** Langue active de l'UI (ex: "fr", "en", "iu") */
  language: string;
  /** Région active (ex: "CA", "US", "FR") */
  region: RegionInfo;
 ...
 */
export function buildLocaleTag(ctx: FormatContext): string {
  ...
  const INTL_FALLBACK: Record<string, string> = {
    iu: "en", ike: "en", ikt: "en",
...
function effectiveTimezone(ctx: FormatContext): string | undefined {
  return ctx.preferences?.timezone || ctx.region.timezone || undefined;
}

/** Format horaire effectif. */
function effective12h(ctx: FormatContext): boolean {
...
export function effectiveMeasurementSystem(ctx: FormatContext): MeasurementSystem {
 ...
 */
export function formatTimeAgo(date: Date | number | string, ctx: FormatContext): string {
  const d = date instanceof Date ? date : new Date(date);
...
/**
 * Conversion métrique ↔ impérial selon le système effectif.
 * Utilise `Intl.NumberFormat` avec `style: "unit"` pour le formatage
 * localisé final (ex: "3 km" en FR, "3 km" en EN, "3 كم" en AR).
 */

type DistanceInput = { meters: number } | { km: number } | { miles: number };
type TemperatureInput = { celsius: number } | { fahrenheit: number };
type MassInput = { kg: number } | { g: number } | { pounds: number };
type VolumeInput = { liters: number } | { ml: number } | { gallons: number };

function convertDistanceToMeters(input: DistanceInput): number {
  if ("meters" in input) return input.meters;
  if ("km" in input) return input.km * 1000;
...
export function formatMass(input: MassInput, ctx: FormatContext): string {
  const system = effectiveMeasurementSystem(ctx);
  const locale = buildLocaleTag(ctx);

  const grams =
    "kg" in input ? input.kg * 1000 :
    "g" in input ? input.g :
...
export function formatVolume(input: VolumeInput, ctx: FormatContext): string {
  const system = effectiveMeasurementSystem(ctx);
  const locale = buildLocaleTag(ctx);

  const ml =
    "liters" in input ? input.liters * 1000 :
