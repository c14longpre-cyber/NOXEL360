...
function buildRegionCatalog() {
    ...
    for (const row of raw) {
        ...
        const measurement = IMPERIAL_REGIONS.has(code)
            ? "imperial"
            : "metric";
        const timeFormat = TWELVE_HOUR_REGIONS.has(code) ? "12h" : "24h";
        const firstDay = FIRST_DAY_EXCEPTIONS[code] !== undefined ? FIRST_DAY_EXCEPTIONS[code] : 1;
        out[code] = {
            code,
...
export const REGION_CATALOG = buildRegionCatalog();
export const ALL_REGIONS = Object.values(REGION_CATALOG).sort((a, b) => a.name.localeCompare(b.name));
