...
if (anchorPattern.test(c)) {
  c = c.replace(anchorPattern, (match, sectionClose, mainClose) => {
    return sectionClose + footerJsx.replace(/\r?\n {8}<\/main>$/, '');
  });
  fs.writeFileSync(p, c, 'utf8');
  console.log('Footer ajouté avec succès à DashboardHome.tsx');
} else {
  console.log('Ancre toujours introuvable. Recherche des occurrences de </section> :');
  const matches = [...c.matchAll(/^( *)<\/section>/gm)];
  matches.forEach((m, i) => console.log(`  Occurrence ${i + 1} : ${m[1].length} espaces d'indentation`));
}
